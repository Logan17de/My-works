...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the ordered shared/add/multiply specialist FFN Transformer."
    )
    ...
    parser.add_argument("--shared-ffn-lr", type=float, default=2e-4)
    parser.add_argument("--add-ffn-lr", type=float, default=2e-4)
    parser.add_argument("--multiply-ffn-lr", type=float, default=2e-4)
...
def checkpoint_payload(
    ...
    return {
        "model_type": OrderedSpecialistTransformer.MODEL_TYPE,
        "architecture_version": OrderedSpecialistTransformer.ARCHITECTURE_VERSION,
        "training_objective": OBJECTIVE_VERSION,
        "model_state": model.state_dict(),
        "optimizer_state": optimizer.state_dict(),
        "model_config": model.config.to_dict(),
        "num_layers": args.layers,
        "plus_token_id": model.plus_token_id,
        "multiply_token_id": model.multiply_token_id,
        "tokenizer": tokenizer.to_dict(),
        "step": int(step),
        "eval_loss": float(eval_loss),
        "train_args": vars(args),
        "parameter_count": model.parameter_count(),
        "ffn_parameter_counts": model.ffn_parameter_counts(),
    }
...
def main() -> None:
    ...
    tokenizer = DynamicByteTokenizer()
    plus_ids = tokenizer.encode("+")
    multiply_ids = tokenizer.encode("*")
    if len(plus_ids) != 1 or len(multiply_ids) != 1:
        raise RuntimeError("Expected '+' and '*' to each map to one tokenizer ID")
    ...
    model = OrderedSpecialistTransformer(
        config,
        num_layers=args.layers,
        plus_token_id=plus_ids[0],
        multiply_token_id=multiply_ids[0],
    ).to(device)
    ...
    shared_ids = {id(p) for p in model.shared_ffn.parameters()}
    add_ids = {id(p) for p in model.add_ffn.parameters()}
    multiply_ids_set = {id(p) for p in model.multiply_ffn.parameters()}
    specialist_ids = shared_ids | add_ids | multiply_ids_set
    backbone_parameters = [
        p for p in model.parameters() if id(p) not in specialist_ids
    ]
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {
                "params": list(model.shared_ffn.parameters()),
                "lr": args.shared_ffn_lr,
                "weight_decay": args.weight_decay,
            },
            {
                "params": list(model.add_ffn.parameters()),
                "lr": args.add_ffn_lr,
                "weight_decay": args.weight_decay,
            },
            {
                "params": list(model.multiply_ffn.parameters()),
                "lr": args.multiply_ffn_lr,
                "weight_decay": args.weight_decay,
            },
        ]
    )
    ...
    print(
        ...
        f"parameters={model.parameter_count():,} device={device}"
    )
    print(
        f"three_reused_ffns={model.ffn_parameter_counts()} "
        f"plus_token_id={model.plus_token_id} "
        f"multiply_token_id={model.multiply_token_id}"
    )
    print(
        f"train_examples={len(train_records)} eval_examples={len(eval_records)} "
        f"max_train_tokens={max(len(row.input_ids) for row in train_records)}"
    )
    ...
    torch.save(
        checkpoint_payload(
            model, optimizer, tokenizer, args, 0, initial_eval
        ),
