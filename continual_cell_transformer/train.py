...
def optimizer_for(model: ContinualCellTransformer, cfg: TrainConfig):
    ...
    return torch.optim.AdamW([
        {"params": parameters, "lr": learning_rates[name], "weight_decay": cfg.weight_decay, "group_name": name}
...
def main():
    ...
    if args.resume:
        ...
        missing, unexpected = model.load_compatible_state_dict(checkpoint["model_state"])
        if missing or unexpected:
            print(f"checkpoint migration: missing={missing} unexpected={unexpected}")
        ...
        cfg = ModelConfig(vocab_size=tokenizer.vocab_size, d_model=args.d_model, n_layers=args.layers, n_heads=args.heads, d_ff=args.d_ff, max_seq_len=args.seq_len, cell_layers=cell_layers, max_cells=args.max_cells, initial_active_cells=args.initial_cells, top_k_cells=args.top_k_cells, max_cell_banks=args.max_cell_banks, new_bank_adapter_scale=args.new_bank_adapter_scale, bank_context_scale=args.bank_context_scale, pad_token_id=tokenizer.pad_token_id, bos_token_id=tokenizer.bos_token_id, eos_token_id=tokenizer.eos_token_id)
        model = ContinualCellTransformer(cfg)

    if args.seq_len > model.config.max_seq_len:
        raise ValueError(f"--seq-len={args.seq_len} exceeds model max_seq_len={model.config.max_seq_len}.")
    ...
    if args.allocate_new_bank > 0:
        model.eval()
        seed_x, _ = sample_batch(train_ids, min(args.batch_size, 32), args.seq_len, device)
        seed_result = model(seed_x)
        allocation = model.allocate_new_bank(args.allocate_new_bank, seed_result["growth_seeds"], not args.do_not_seal_existing_banks)
        structural_change = True
        print(f"one-time bank allocation={allocation}")
        print(f"bank summaries={model.bank_summaries()}")

    train_cfg = TrainConfig(steps=args.steps, batch_size=args.batch_size, eval_interval=args.eval_interval, eval_batches=args.eval_batches, log_interval=args.log_interval, weight_decay=args.weight_decay, grad_clip=args.grad_clip, embedding_lr=args.embedding_lr, attention_lr=args.attention_lr, ffn_lr=args.ffn_lr, cell_lr=args.cell_lr, other_lr=args.other_lr, mature_cell_scale=args.mature_cell_scale, enable_growth=args.enable_growth, growth_warmup_steps=args.growth_warmup, growth_patience=args.growth_patience, growth_cells=args.growth_cells, growth_confidence=args.growth_confidence, growth_loss_floor=args.growth_loss_floor, max_growth_events=args.max_growth_events, growth_cooldown_steps=args.growth_cooldown, seed=args.seed)
    optimizer = optimizer_for(model, train_cfg)

    if args.resume and not structural_change and checkpoint is not None and "optimizer_state" in checkpoint:
        try:
        ...
        print("optimizer state restarted after vocabulary/bank structural change")

    initial_eval = evaluate(model, eval_ids, args.batch_size, args.seq_len, device, args.eval_batches)
    print(f"initial eval loss={initial_eval:.4f} ppl={math.exp(min(initial_eval, 20)):.2f} file={args.eval_file or args.train_file}")
    ...
    if args.retention_file:
        ...
        retention_before = evaluate(model, retention_ids, args.batch_size, args.seq_len, device, args.eval_batches)
    ...
    for local_step in range(1, args.steps + 1):
        ...
        growth_ready = args.enable_growth and growth_events < args.max_growth_events and local_step >= args.growth_warmup and local_step - last_growth_step >= args.growth_cooldown
        if growth_ready and loss_ema > args.growth_loss_floor and confidence < args.growth_confidence:
            low_confidence += 1
        else:
            low_confidence = 0
        if growth_ready and low_confidence >= args.growth_patience:
            allocated = model.allocate_new_bank(args.growth_cells, result["growth_seeds"], True)
            growth_events += 1
            last_growth_step = local_step
            low_confidence = 0
            print(f"growth event={growth_events} allocation={allocated} step={step} loss_ema={loss_ema:.4f} confidence={confidence:.3f}")
            print(f"bank summaries={model.bank_summaries()}")
        if local_step == 1 or local_step % args.log_interval == 0:
            print(f"step={step} loss={loss:.4f} ppl={math.exp(min(loss, 20)):.2f} confidence={confidence:.3f} banks={result['cell_banks']} top_ids={result['bank_top_ids']}")
        if local_step % args.eval_interval == 0 or local_step == args.steps:
            final_eval = evaluate(model, eval_ids, args.batch_size, args.seq_len, device, args.eval_batches)
            print(f"eval step={step} loss={final_eval:.4f} ppl={math.exp(min(final_eval, 20)):.2f} file={args.eval_file or args.train_file}")

    if args.seal_active_cells:
        model.seal_active_cells()
    retention_after = None
    if retention_ids is not None:
        retention_after = evaluate(model, retention_ids, args.batch_size, args.seq_len, device, args.eval_batches)
        print(f"old-task loss after={retention_after:.4f}; delta={retention_after - retention_before:+.4f}")

    payload = {"model_state": model.state_dict(), "optimizer_state": optimizer.state_dict(), "model_config": model.config.to_dict(), "tokenizer": tokenizer.to_dict(), "step": previous_step + args.steps, "train_args": vars(args), "architecture_version": 2}
    torch.save(payload, out / "checkpoint.pt")
    tokenizer.save(out / "tokenizer.json")
    summary = {"step": payload["step"], "architecture_version": 2, "vocab_size": tokenizer.vocab_size, "active_cells": [pool.active_count for pool in model.pools()], "cell_banks": model.bank_summaries(), "train_file": args.train_file, "eval_file": args.eval_file or args.train_file, "initial_eval_loss": initial_eval, "final_eval_loss": final_eval, "retention_file": args.retention_file, "retention_before": retention_before, "retention_after": retention_after, "growth_events": growth_events}
