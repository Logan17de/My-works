#!/usr/bin/env python3
"""Run the complete Animation Engine from one command with clear stage progress."""
...
def main():
    ...
    p.step("Retargeting ARDY motion onto the rigged character")
    final_fbx=out/"character_animated.fbx"; preview_glb=out/"character_animated_preview.glb"
    run(["/opt/conda/bin/conda","run","--no-capture-output","-n","mia","python",str(tools/"retarget_with_mia.py"),"--target",str(rigged),"--animation",str(ardy_source),"--output",str(final_fbx),"--preview-glb",str(preview_glb),"--fps",str(fps)],p,"Auto-Rig-Pro retarget/export",env=mia_env)
    ...
    package_manifest={"animation_master":final_fbx.name,"material_master":material_source.name if material_source else None,"animated_preview_glb":preview_glb.name if preview_glb.is_file() else None,"source_asset_manifest":source_manifest_name,"motion_preview":motion_preview.name,"contract_report":report.name,"fps":fps,"unreal_note":"Import FBX as Skeletal Mesh/Animation. Use automatic/custom sample rate matching fps. Reapply PBR from the material-master GLB when present."}
    ...
    print("PACKAGE_ZIP="+archive,flush=True); print("FINAL_FBX="+str(final_fbx),flush=True); print("MOTION_PREVIEW="+str(motion_preview),flush=True); print("CONTRACT_REPORT="+str(report),flush=True)
