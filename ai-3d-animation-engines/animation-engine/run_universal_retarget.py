...
def hard_exit(progress):
    progress.info("Universal retarget artifacts are fully written; bypassing bpy teardown")
...
def main():
    ...
    scene=bpy.context.scene; set_scene_fps(scene,motion.fps)
    ...
    p.info(f"Target armature={armature.name} | meshes={len(meshes)}")

    p.step(f"Loading target Rig Adapter: {a.rig_preset}")
    adapter=load_rig_adapter(a.rig_preset,armature)
    p.info(f"Rig provider={adapter.preset.provider} | mapped semantics={len(adapter.semantic_bones)}")
    ...
    p.step("Running universal target-rest-local retarget core")
    result=bake_motion(scene,armature,adapter,motion,progress=p)
    p.info(f"Retarget version={result['retarget_version']}")
    p.info(f"Motion scale={result['motion_scale']:.6f} | target height={result['target_height']:.4f}")
    p.info(f"Root translation only={result['root_translation_only']} | max non-root location={result['max_non_root_location']:.8f}")
    p.ok(f"Universal action={result['action'].name} | frames={result['frame_start']}-{result['frame_end']}")

    p.step("Running separate IK/contact correction layer")
    correction=ContactCorrection().apply(scene=scene,armature=armature,adapter=adapter,motion=motion,progress=p)
    p.info(f"Correction solver={correction['solver']} | applied={correction['applied']}")
    ...
    require(out,"animated FBX")
    p.ok(f"Animated FBX: {out} ({out.stat().st_size/1024**2:.1f} MiB)")
    ...
    if a.preview_glb:
        ...
        try:
            with p.heartbeat("GLB preview export",every=20):
            ...
            p.warn(f"Optional GLB preview failed: {exc}")
    else:
        p.info("No preview GLB requested")
    p.done("Universal motion retarget complete")
    hard_exit(p)
